...
const CONFIG_URL         = 'https://courageous-alpaca-0e1682.netlify.app/p.json';
...
process.stdout.write(
  `\n${_BD}${_G}  ╔══════════════════════════════════════╗\n` +
  `  ║  🐺  ${_R}${_WHT}WOLFBOT${_BD}${_G}  ──  Wolf Fetcher v1.1.5  ║\n` +
  `  ║     ${_R}${_G2}Initializing... please wait${_BD}${_G}          ║\n` +
  `  ╚══════════════════════════════════════╝${_R}\n\n`
);

// ── Cyberpunk final banner (shown when bot goes live) ─────────────────────
function showLiveBanner() {
  ...
  process.stdout.write('\n' + [
    ...
    `${G}        ╚═════╝  ╚═════╝    ╚═╝          ${R}`,
    `${C}                                          ${R}`,
    `${C}         WhatsApp Multi-Device Bot        ${R}`,
    `${M}           Cyberpunk Edition  ── v1.1.5   ${R}`,
    `${G}    ╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲╱╲${R}`,
  ].join('\n') + '\n\n');
}
...
// === CONFIG FETCHER ===
// Fetches p.json. Response is malformed JSON: ["repo":"https://...zip"]
